...
int intel_scu_ipc_read_osnib(u8 * data, int len, int offset)
{
	...
	if (check) {
		pr_err
		    ("WARNING!!! osnib chksum verification faild, reset all osnib data!\n");
		struct_offs =
		    offsetof(struct scu_ipc_oshob,
			     osnibw_ptr) + oshob_info->offs_add;
...
int intel_scu_ipc_write_osnib(u8 * data, int len, int offset)
{
	...
	if (osnibw_ptr == 0) {	/* workaround here for BZ 2914 */
		...
		pr_err
		    ("ERR: osnibw ptr from oshob is 0, manually set it here\n");
	}
...
int intel_scu_ipc_read_osnib_extend(u8 * data, int len, int offset)
{
	...
	pr_debug("ipc_read_osnib_extend: remap OSNIB addr=0x%x size %d\n",
	...
	if (check) {
		pr_err
		    ("ipc_read_osnib_extend: WARNING!!! osnib chksum verification faild, reset all osnib data!\n");
		pr_debug
		...
		if (!osnibw_addr) {
			pr_err
			    ("ipc_read_osnib_extend: cannot remap osnib write ptr\n");
			goto unmap_oshob_addr;
		}
...
int intel_scu_ipc_write_osnib_extend(u8 * data, int len, int offset)
{
	...
	pr_debug("ipc_write_osnib_extend: remap OSHOB addr 0x%8x size %d\n",
	...
	if (posnib_data == NULL) {
		pr_err
		    ("ipc_write_osnib_extend: The buffer for getting OSNIB is NULL\n");
		ret = -EFAULT;
		goto unmap_oshob_addr;
	}
	...
	if (check) {
		pr_err
		    ("ipc_write_osnib_extend: WARNING!!! OSNIB data chksum verification FAILED!\n");
	} else {
		/* generate chksum.  */
	...
	pr_debug("ipc_write_osnib_extend: remap osnibw ptr addr=0x%x size %d\n",
		 oshob_info->osnibw_ptr, oshob_info->osnib_size);
	...
	if (ret < 0)
		pr_err
		    ("scu_ipc_write_osnib_extend: ipc_write_osnib failed!!\n");
...
int intel_scu_ipc_write_osnib_rr(u8 rr)
{
	...
	return oshob_info->scu_ipc_write_osnib(&rr,
					       1,
					       offsetof(struct scu_ipc_osnib,
							target_mode));
}
...
int intel_scu_ipc_read_osnib_rr(u8 * rr)
{
	...
	return oshob_info->scu_ipc_read_osnib(rr,
					      1,
					      offsetof(struct scu_ipc_osnib,
						       target_mode));
}
...
int intel_scu_ipc_read_oshob_extend_param(void __iomem * poshob_addr)
{
	...
	oshob_info->osnib_size = readw(poshob_addr +
	...
	oshob_info->oemnib_size = readw(poshob_addr +
					offsetof(struct scu_ipc_oshob_extend,
						 oem_size));
	...
	intel_scu_ipc_read_oshob((u8 *) (oshob_info->scu_trace),
				 buff_size,
	...
	if (!oshob_info->osnibr_ptr) {
		pr_err
		    ("ipc_read_oshob_extend_param: R_INTEL_POINTER is NULL!\n");
		return -ENOMEM;
	}

	struct_offs =
	    offsetof(struct scu_ipc_oshob_extend,
		     w_intel_ptr)+oshob_info->offs_add;
	...
	oshob_info->scu_ipc_write_osnib = &intel_scu_ipc_write_osnib_extend;
	oshob_info->scu_ipc_read_osnib = &intel_scu_ipc_read_osnib_extend;

	pr_info("Using EXTENDED OSHOB structure size = %d bytes\n",
		oshob_info->oshob_size);
	pr_info("OSNIB Intel size = %d bytes OEMNIB size = %d bytes\n",
...
int intel_scu_ipc_read_oshob_extend_param_v14(void __iomem * poshob_addr)
{
	...
	if (!oshob_info->osnibr_ptr) {
		pr_err
		    ("ipc_read_oshob_extend_param_v14: R_INTEL_POINTER is NULL!\n");
		return -ENOMEM;
	}
	...
	pr_info("Using latest extended oshob structure size = %d bytes\n",
		oshob_info->oshob_size);
	pr_info("OSNIB Intel size = %d bytes OEMNIB size = %d bytes\n",
		oshob_info->osnib_size, oshob_info->oemnib_size);
...
int intel_scu_ipc_read_oshob_def_param(void __iomem * poshob_addr)
{
	...
	ret = intel_scu_ipc_read_oshob((u8 *) (oshob_info->scu_trace),
				       buff_size,
	...
	struct_offs =
	    offsetof(struct scu_ipc_oshob, iatxl)+oshob_info->offs_add;
	oshob_info->ia_trace = readl(poshob_addr + struct_offs);

	oshob_info->scu_ipc_write_osnib = &intel_scu_ipc_write_osnib;
	oshob_info->scu_ipc_read_osnib = &intel_scu_ipc_read_osnib;
...
int intel_scu_ipc_read_oshob_info(void)
{
	...
	if (oshob_info->platform_type == INTEL_MID_CPU_CHIP_TANGIER) {
		pr_info
		    ("(oshob) identified platform = INTEL_MID_CPU_CHIP_TANGIER\n");
	...
	pr_debug("(oshob) OSHOB magic = %x %x %x %x\n",
		 oshob_magic[0], oshob_magic[1], oshob_magic[2],
		 oshob_magic[3]);
	...
	if (strncmp(oshob_magic, OSHOB_MAGIC_NUMBER,
		...
		oshob_info->oshob_majrev = readb(oshob_addr + offsetof(struct
								       scu_ipc_oshob_extend,
								       rev_major));
		...
		if ((oshob_info->oshob_majrev >= 1) &&
			...
			if (intel_scu_ipc_read_oshob_extend_param(oshob_addr) !=
			    0) {
				ret = -EFAULT;
				goto unmap_oshob;
			}
...
int intel_scu_ipc_write_oemnib(u8 * oemnib, int len, int offset)
{
	...
	if (oemnib == NULL) {
		pr_err
		    ("ipc_write_oemnib: passed buffer for writting OEMNIB is NULL\n");
		return -EINVAL;
	}
	...
	/* So the mask is coded on 4 bytes.                             */
	sptr_dw_mask = 0xFFFFFFFF;
	if ((oshob_info->oshob_majrev >= 1) && (oshob_info->oshob_minrev >= 4)) {
		sptr_dw_mask = 0xFFFFFFFF;
...
static int intel_scu_ipc_read_oemnib(u8 * oemnib, int len, int offset)
{
	...
	if (oemnib == NULL) {
		pr_err
		    ("ipc_read_oemnib: passed buffer for reading OEMNIB is NULL\n");
		return -EINVAL;
	}
	...
	if ((len == 0) || (len > oshob_info->oemnib_size)) {
		pr_err
		    ("ipc_read_oemnib: Bad OEMNIB data length (%d) to be read (max=%d bytes)\n",
		     len, oshob_info->oemnib_size);
		ret = -EINVAL;
		goto unmap_oshob_addr;
	}
...
int intel_scu_ipc_cmd_oemnib(int cmd, const char *arg)
{
	...
	if (arg != NULL) {
		...
		if (length > oshob_info->oemnib_size) {
			printf
			    ("write size exceeds oemnib size. ignore exceeded data.\n");
	...
	if (cmd)
		ret = intel_scu_ipc_read_oemnib(oemnib_buf,
						oshob_info->oemnib_size, 0);
	else
		ret = intel_scu_ipc_write_oemnib(oemnib_buf, length, 0);
...
static int intel_scu_ipc_read_oshob_it_tree(u32 * ptr)
{
	...
	return intel_scu_ipc_read_oshob((u8 *) ptr, 4, struct_offs);
}
#endif
...
static int intel_scu_ipc_read_osnib_reset_ev1(u8 * rev1)
{
	...
	for (i = 0; i < ARRAY_SIZE(chip_reset_events); i++) {
		if (chip_reset_events[i].id == oshob_info->platform_type) {
			pr_debug("intel_scu_ipc_read_osnib_rst_ev1: read %s\n",
				 chip_reset_events[i].reset_ev1_name);

			return oshob_info->scu_ipc_read_osnib(rev1,
							      1,
							      offsetof(struct
								       scu_ipc_osnib,
								       reset_ev1));
...
static int intel_scu_ipc_read_osnib_reset_ev2(u8 * rev2)
{
	...
	for (i = 0; i < ARRAY_SIZE(chip_reset_events); i++) {
		if (chip_reset_events[i].id == oshob_info->platform_type) {
			pr_debug("intel_scu_ipc_read_osnib_rst_ev2: read %s\n",
				 chip_reset_events[i].reset_ev2_name);

			return oshob_info->scu_ipc_read_osnib(rev2,
							      1,
							      offsetof(struct
								       scu_ipc_osnib,
								       reset_ev2));
...
int intel_scu_ipc_read_osnib_wd(u8 * wd)
{
	...
	return oshob_info->scu_ipc_read_osnib(wd,
					      1,
					      offsetof(struct scu_ipc_osnib,
						       wd_count));
}
...
int intel_scu_ipc_write_osnib_wd(u8 * wd)
{
	...
	return oshob_info->scu_ipc_write_osnib(wd,
					       1,
					       offsetof(struct scu_ipc_osnib,
							wd_count));
}
...
u32 intel_scu_ipc_get_fabricerror_buf1_offset(void)
{
	...
	if (oshob_info->platform_type == INTEL_MID_CPU_CHIP_CLOVERVIEW)
		...
		pr_err
		    ("scu_ipc_get_fabricerror_buf_offset: platform not recognized!\n");
		return 0;
	}
}
...
static int intel_scu_ipc_read_osnib_alarm(u8 * alarm)
{
	...
	return oshob_info->scu_ipc_read_osnib(alarm,
					      1,
					      offsetof(struct scu_ipc_osnib,
						       alarm));
}
...
static int intel_scu_ipc_read_osnib_wakesrc(u8 * wksrc)
{
	...
	return oshob_info->scu_ipc_read_osnib(wksrc,
					      1,
					      offsetof(struct scu_ipc_osnib,
						       wakesrc));
}
...
int intel_scu_ipc_oshob_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(chip_reset_events); i++) {
		if (chip_reset_events[i].id == oshob_info->platform_type) {
			pr_warn
			    ("[BOOT] %s=0x%02x %s=0x%02x (PMIT interrupt tree)\n",
			     chip_reset_events[i].reset_ev1_name, reset_ev1,
			     chip_reset_events[i].reset_ev2_name, reset_ev2);
		}
	}
	...
	if ((oshob_info->oshob_majrev == OSHOB_REV_MAJ_DEFAULT) &&
		...
		ret = intel_scu_ipc_read_oshob((u8 *) (scu_trace),
					       buff_size,
					       offsetof(struct scu_ipc_oshob,
							scutxl));
		...
		struct_offs = offsetof(struct scu_ipc_oshob, iatxl) +
		    oshob_info->offs_add;
		ret = intel_scu_ipc_read_oshob((u8 *) (&ia_trace),
					       4, struct_offs);
	...
	for (i = 0; i < ARRAY_SIZE(chip_reset_events); i++) {
		if (chip_reset_events[i].id == oshob_info->platform_type) {
			pr_warn
			    ("[BOOT] WAKESRC=0x%02x %s=0x%02x %s=0x%02x (osnib)\n",
			     wakesrc, chip_reset_events[i].reset_ev1_name,
			     reset_ev1, chip_reset_events[i].reset_ev2_name,
			     reset_ev2);
		}
	}
...
struct secure_rtc_tod *intel_scu_ipc_read_srtc(void)
{
	...
	printf
	    ("ipc_mb opcode = 0x%02x, status 0x%02x ostype 0x%02x oper 0x%02x\n",
	     ipc_mb.opcode, ipc_mb.status, ipc_mb.ostype, ipc_mb.oper);
	status =
	    intel_scu_ipc_command(OEM_IPC_COMMAND, 0, (u8 *) & ipc_mb, 8,
				  result, 4);
	if (status < 0) {
...
int intel_scu_ipc_verify_img(u8 imgtype, u8 keyindex, u32 ddraddr)
{
	...
	printf
	    ("ipc_mb opcode = 0x%02x, status 0x%02x ostype 0x%02x oper 0x%02x addr 0x%08x\n",
	     ipc_mb.opcode, ipc_mb.status, ipc_mb.ostype, ipc_mb.oper,
	     ipc_mb.data[0]);
	status =
	    intel_scu_ipc_command(OEM_IPC_COMMAND, 0, (u8 *) & ipc_mb, 8,
