...
#define SDHCI_CONTROLLER_BASE_ADDR	CONFIG_SYS_EMMC_PORT_BASE
...
{
	unsigned int base = 0;
	u32 mmc_pci = PCI_BDF(0, 23, 0);	/* eMMC 4.5 */
	pci_read_config_dword(mmc_pci, 0x10, &base);
	base &= 0xffffe000;
	if (base == 0xffffe000 || base == 0x0) {
		printf("No eMMC4.5 BAR found in PCI\n");
		mmc_pci = PCI_BDF(0, 16, 0);	/* eMMC 4.41 */
		pci_read_config_dword(mmc_pci, 0x10, &base);
		base &= 0xffffe000;
	} else {
		printf("eMMC4.5 BAR found in PCI(0x%08x)\n", base);
		goto found;
	}

	if (base == 0xffffe000 || base == 0x0) {
		printf("No eMMC4.41 BAR found in PCI\n");
	} else {
		printf("eMMC4.41 BAR found in PCI(0x%08x)\n", base);
		goto found;
	}

	if (base == 0xffffffff || base == 0xffffe000 || base == 0x0) {
		base = CONFIG_SYS_EMMC_PORT_BASE;
		printf("No eMMC BAR found at all\n");
		printf("Use hard-coded eMMC BAR(0x%08x)\n", CONFIG_SYS_EMMC_PORT_BASE);
	}
found:
	base += (0x40000 * index);
